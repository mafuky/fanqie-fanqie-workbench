...
7. 撰写正文 - **每章必须达到3000-5000字**
   ...
   - **标点节制（硬约束）**：
     - 全章中文破折号 `——` **总数不得超过 20 个**（每千字最多 4 个）
     - **严禁**把 `——` 单独成行作为段落装饰（如 `\n——\n` 节奏标点）
     - **严禁**连续两段之间用孤立 `——` 行分隔
     - `——` 只用于：人物语言被打断、内心补充说明、定义解释三种合法场景
     - 同理省略号 `……` 全章不超过 30 个
     - 用「换段」「短句」「场景切换」表现节奏，不靠特殊标点撑节奏
...
11. **深度润色（去除AI味）** - 重点检查并修改：
    - **标点 AI 味自检（必做，硬指标）**：
      - 用 `grep -c '——' <章节文件>` 数破折号，**> 20 个必须删减到 ≤ 20**
      - 用 `grep -nE '^——$' <章节文件>` 找单独成行的破折号，**全部删除**
      - 用 `grep -c '……' <章节文件>` 数省略号，**> 30 个必须删减**
      - 用 `grep -cE '^(然而|但是|不过|不仅|而且)' <章节文件>` 数转折词，每千字 > 5 必须改写
